...
class CumulativeTrainer(object):
    ...
    def train_epoch(self, method, train_dataset, train_collate_fn, batch_size, epoch, optimizer, scheduler=None):
        ...
        if torch.cuda.is_available():
            ...
            train_loader = torch.utils.data.DataLoader(train_dataset, collate_fn=train_collate_fn,
        ...
        for j, data in enumerate(train_loader, 0):
            ...
            if j > 0:
                ...
                if scheduler is not None:
                    print('Method', method, 'Epoch', epoch, 'Batch ', count_batch, 'Loss ', bloss, 'Time ',
                          elapsed_time, 'Learning rate ', scheduler.get_lr())
                else:
                    print('Method', method, 'Epoch', epoch, 'Batch ', count_batch, 'Loss ', bloss, 'Time ',
                          elapsed_time)
